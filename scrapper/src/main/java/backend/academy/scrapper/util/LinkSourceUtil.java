...
public class LinkSourceUtil {
    ...
    public static LinkType getLinkType(URI uri) {
        String url = uri.toString();
        String host = uri.getHost();
        return getLinkType(host, url).orElse(LinkType.GITHUB); // Default to GitHub if can't determine
    }

    private static boolean isSupportedSource(LinkType linkType, String url) {
        return regex.get(linkType).stream().anyMatch(it -> Pattern.matches(domains.get(linkType) + it, url));
    }
    ...
    private Map<LinkType, List<String>> initRegexMap(ScrapperConfig applicationConfig) {
        return applicationConfig.linkSources().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().handlers().values().stream()
    ...
    private Map<LinkType, String> initDomainMap(ScrapperConfig applicationConfig) {
        return applicationConfig.linkSources().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().domain()));
    }
}
